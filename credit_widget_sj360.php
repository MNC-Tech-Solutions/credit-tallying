...
try {
    $db  = getDb();
    $sql = "SELECT location_id, location_name, ABS(IFNULL(amount, 0)) AS amount, tx_date
            FROM transactions
            WHERE type = 'Workflow - Premium Features'";
    $params = [];
    if ($targetLocationId !== '') {
        $sql    .= ' AND location_id = ?';
    ...
    while ($row = $stmt->fetch()) {
        ...
        $locationName = $row['location_name'] ?: $locationId;
        ...
        if ($dateStr !== '') {
            ...
            if (!$date) $date = DateTime::createFromFormat('Y-m-d H:i:s', $dateStr);
            if (!$date) $date = DateTime::createFromFormat('Y-m-d', $dateStr);
            if ($date) {
                $mk = $date->format('Y-m');
    ...
    echo '<html><body><div style="text-align:center;padding:40px;font-family:sans-serif;"><h2>Database error: ' . htmlspecialchars($e->getMessage()) . '</h2></div></body></html>'; exit;
}
...
if (empty($processedData)) {
    echo '<html><body><div style="text-align:center;padding:40px;font-family:sans-serif;"><h2>No data found.</h2></div></body></html>'; exit;
}
